...
batch_size = 12
num_iters= 30000
num_epochs = 30#num_iters / (len(combined_train_dataset) / batch_size)
...
combined_model = MultimodalClassifier_BA(num_classes=num_classes)
...
for epoch in range(num_epochs):
    ...
    for batch_idx, batch_data in enumerate(tqdm(combined_train_loader, total = len(combined_train_loader), desc = "Training")):
        ...
        logits, _, _ = combined_model(images, input_ids, attention_mask)
    ...
    with torch.no_grad():
        for valid_batch_data in combined_valid_loader:
            ...
            valid_logits, _, _ = combined_model(valid_images, valid_input_ids, valid_attention_mask)
    ...
    if callback_list.on_epoch_end(epoch+1,epoch_accuracy):
            ...
            counter = 0
            torch.save(combined_model.state_dict(),'Combine_best_model_BA.pt')
...
combined_model.load_state_dict(torch.load('Combine_best_model_BA.pt'))
print('Model Loaded')
...
with torch.no_grad():
    for ctest_batch_data in combined_test_loader:
        ...
        ctest_logits, _, _ = combined_model(ctest_images, ctest_input_ids, ctest_attention_mask)
...
plt.savefig("basic_attention_confusion_matrix.png")
...
plt.savefig("basic_attention_train_val_loss.png")
